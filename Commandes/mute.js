...
module.exports = {
    ...
    name: "mute", 
    description: "→ Permets de Mute un membre !",
    ...
    async run(bot, message, args) {
        ...
        if(ms(time) > 86400000) return message.reply("Le mute ne peut pas durer plus de 28jours !")

        let reason = args.getString("raison")
        if(!reason) reason = "Pas de raison fournie.";

        if(message.user.id === user.id) return message.reply("Ne te mute pas tout seul !")
        if((await message.guild.fetchOwner()).id === user.id) return message.reply("Ne mute pas le fondateur")
        if(!member.moderatable) return message.reply("Je ne peut pas mute cette personne")
